...
{      
    ...
    if(HAL_RTCEx_BKUPRead(&RTC_Handler,RTC_BKP_DR0)!=0X5040)//是否第一次配置
    { 
        RTC_Set_Time(15,45,56,RTC_HOURFORMAT12_PM);	        //设置时间 ,根据实际时间修改
		RTC_Set_Date(25,8,20,3);		                    //设置日期
        HAL_RTCEx_BKUPWrite(&RTC_Handler,RTC_BKP_DR0,0X5040);//标记已经初始化过了
    }
    return 0;
}
